...
export default async function handler(req: NextRequest, res: any) {
  ...
  const indianCuisineId = cuisines.find((cuisine) => cuisine.name === "indian")?.id ?? "";
  const mexicanCuisineId = cuisines.find((cuisine) => cuisine.name === "mexican")?.id ?? "";
  const italianCuisineId = cuisines.find((cuisine) => cuisine.name === "italian")?.id ?? "";

  const ottawaLocationId = locations.find((location) => location.name === "ottawa")?.id ?? "";
  const torontoLocationId = locations.find((location) => location.name === "toronto")?.id ?? "";
  const niagaraLocationId = locations.find((location) => location.name === "niagara")?.id ?? "";
  ...
  await prisma.restaurant.createMany({
    data: [
      // INDIAN //
      {
        ...
        openTime: "14:30:00.000+10:00",
        closeTime: "21:30:00.000+10:00",
      ...
      },
      {
        ...
        openTime: "12:30:00.000+10:00",
        closeTime: "22:00:00.000+10:00",
      ...
      },
      {
        ...
        openTime: "10:00:00.000+10:00",
        closeTime: "21:00:00.000+10:00",
      ...
      },
      {
        ...
        openTime: "14:00:00.000+10:00",
        closeTime: "19:00:00.000+10:00",
        slug: "utsav-toronto",
        locationId: torontoLocationId,
        cuisineId: indianCuisineId,
      },
  ...
  const vivaanId =
    restaurants.find((restaurant) => restaurant.name === "Vivaan - fine Indian")?.id ?? "";
  const RamaKrishnaId =
    restaurants.find((restaurant) => restaurant.name === "RamaKrishna Indian")?.id ?? "";
  const coconutLagoonId =
    restaurants.find((restaurant) => restaurant.name === "Coconut Lagoon")?.id ?? "";
  const lastTrainToDelhiId =
    restaurants.find((restaurant) => restaurant.name === "Last Train to Delhi")?.id ?? "";
  ...
  const utsavId = restaurants.find((restaurant) => restaurant.name === "Utsav")?.id ?? "";
  const pukkaId = restaurants.find((restaurant) => restaurant.name === "Pukka")?.id ?? "";
  const kamasutraIndianId =
    restaurants.find((restaurant) => restaurant.name === "Kamasutra Indian")?.id ?? "";
  const eldoradoTacoId =
    restaurants.find((restaurant) => restaurant.name === "Eldorado Taco")?.id ?? "";
  const laBartolaId = restaurants.find((restaurant) => restaurant.name === "La Bartola")?.id ?? "";
  const elCatrinId = restaurants.find((restaurant) => restaurant.name === "El Catrin")?.id ?? "";
  ...
  const stelvioId = restaurants.find((restaurant) => restaurant.name === "Stelvio")?.id ?? "";
  const sofiaId = restaurants.find((restaurant) => restaurant.name === "Sofia")?.id ?? "";
