...
import RestaurantBooking from "@/app/restaurant/[slug]/components/RestaurantBooking";
...
const RestaurantPage = async ({ params: { slug } }: RestaurantPageProps) => {
  const { name, description, images, reviews, openTime, closeTime } = await useRestaurant(slug);
  ...
  return (
    <>
      ...
      {/* left aside end */}
      {/* right start */}
      <RestaurantBooking slug={slug} openTime={openTime} closeTime={closeTime} />
      {/* right end */}
    </>
  );
};
